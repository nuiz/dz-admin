...
<table class="table table-bordered table-dz">
    ...
    <tr style="<?php if($item->admin_read==1) echo "background: #CCCCCC;";?>">
        <td><?php echo "<a href='".URL::to("popup/regupgrade/{$item->id}")."' class='popup-button-fancy fancybox.ajax'>{$item->name}</a>";?></td>
...
$(function(){
    $.get('<?php echo URL::to('notification/read');?>', { type: "register_upgrade" }, function(data){
    ...
    $(".check-call").change(function(e){
        ...
        $.post("<?php echo URL::to("notification/editregup");?>/"+object_id, { called: called }, function(data){
