...
<form method="post" class="add-showcase-form create-form" style="padding: 10px;">
    <legend>{{ $header }}</legend>
    <div class="form-group">
        <label>youtube id</label>
        <input type="text" name="youtube_id" class="form-control" value="{{ @$post['youtube_id'] }}" readonly>
    </div>
    <div class="form-group">
        <label>name</label>
        <input type="text" name="name" class="form-control" value="{{ @$post['youtube_id'] }}" readonly>
    </div>
    <div class="form-group">
        <label>description</label>
        <textarea name="description" class="form-control" readonly>{{ @$post['youtube_id'] }}</textarea>
    </div>
    <div class="checkbox">
        <label>
            <input type="checkbox" name="to_feed" value="true" <?php if(isset($post['to_feed'])){ echo "checked"; }?>> post to feed
        </label>
    </div>
    <div class="hidden">
        <input type="hidden" name="duration" value="{{ @$post['youtube_id'] }}">
        <input type="hidden" name="thumb" value="{{ @$post['youtube_id'] }}">
        <input type="hidden" name="like_count" value="{{ @$post['youtube_id'] }}">
        <input type="hidden" name="view_count" value="{{ @$post['youtube_id'] }}">
        <input type="hidden" name="comment_count" value="{{ @$post['youtube_id'] }}">
    </div>
...
$(function(){
    (function(){
        <?php if(Input::getMethod()=="post"){ echo "return true;"; } ?>
        ...
        var form = $('.add-showcase-form');
        var youtube_id = youtube_parser("<?php echo Input::get("youtube_link");?>");
        ...
        $.get("https://www.googleapis.com/youtube/v3/videos?id="+youtube_id+"&key=AIzaSyA1reRMMGfDbsiJzqUvXWaNgQrmGdaCAk8&part=snippet,contentDetails,statistics", function(data){
            ...
            $('input[name="youtube_id"]', form).val(video.id);
            ...
            $('input[name="name"]', form).val(name).prop('readonly', false);
            $('textarea[name="description"]', form).val(description).prop('readonly', false);

            $('input[name="duration"]', form).val(duration);
            $('input[name="thumb"]', form).val(thumb);
            $('input[name="like_count"]', form).val(like_count);
            $('input[name="view_count"]', form).val(view_count);
            $('input[name="comment_count"]', form).val(comment_count);
