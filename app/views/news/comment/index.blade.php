...
<div style="width: 660px; margin: 0 auto; padding: 10px 0; background: white;">
    <div class="news-block display-block">
        <?php
        if($news->media_type == "picture"){?>
        <div>
            <img src="http://61.19.147.72/api/pic/<?php echo $news->picture_id;?>?display=custom&size_x=640">
        </div>
        <?php } else if($news->media_type == "video"){?>
        <div>
            <video width="320" height="240">
                <source src="<?php echo $news->video->link;?>" type="video/mp4">
                Your browser does not support the video tag.
            </video>
        </div>
        <?php }?>
        <p>
            <?php echo nl2br($news->message);?>
        </p>
    </div>
